...
import { MessageCircle, Send, Bot, Users, Bell, User, UserPlus, AlertCircle, CheckCircle, Wrench, Clock, Reply, Trash2, X } from 'lucide-react';
...
const StudentChat = () => {
  const [activeMode, setActiveMode] = useState('peer');
  ...
  const initializeUser = async (userId, name) => {
    try {
      await axios.post(`${API_URL}/session/create`, {
        user_id: userId,
        username: name
      });
  ...
  const handleLogin = async (e) => {
    ...
    if (!username.trim() || username.trim().length < 2 || !usernameValid) {
  ...
  const handleDelete = async (message) => {
    ...
    try {
      await axios.delete(`${API_URL}/peer/message/${message.id}?user_id=${currentUser.id}`);
    ...
    <div className="student-chat-container">
      ...
      {contextMenu && (
        ...
        >
          ...
          {contextMenu.message.sender === 'user' && !contextMenu.message.deleted && (
            <button 
                        ...
                        <div className="message-content">
                          ...
                          {message.reply_to_id && (
                            <div className="reply-preview">
                              ...
                              <span>{getRepliedMessage(message.reply_to_id)?.text || 'Original message'}</span>
                            </div>
                          )}
                          
                          <div className={`message-bubble ${message.deleted ? 'deleted-message' : ''}`}>
                    ...
                    {!selectedPeer ? (
                      ...
                      <>
                        {replyingTo && (
                          <div className="replying-to-banner">
                            ...
                            <div className="flex-1">
                              <p className="text-xs text-gray-500">Replying to {replyingTo.username || 'yourself'}</p>
                        ...
                        <p className="text-xs text-gray-500 mt-2">
                          Press Enter to send • Right-click message to reply/delete
                        </p>
                      </>
                    )}
                  </div>
...
// // should be removed later 2nd since it is for AI being worked on
