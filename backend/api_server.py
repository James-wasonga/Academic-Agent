...
def gemini_research(query: str) -> dict:
    ...
    try:
        ...
        # ✅ Try -latest, fallback to base model if unavailable
        model_name = "gemini-1.5-flash-latest"
        try:
            model = genai.GenerativeModel(model_name)
            print(f"✅ Using model: {model_name}")
        except Exception:
            print("⚠️ gemini-1.5-flash-latest unavailable, falling back to gemini-1.5-flash")
            model_name = "gemini-1.5-flash"
            model = genai.GenerativeModel(model_name)
            print(f"✅ Now using fallback model: {model_name}")
        ...
        Make the response comprehensive, professional, and academically sound.
        """
        ...
        return {
            ...
            "summary": formatted_summary,
            "sources": "Comprehensive AI analysis utilizing advanced language models trained on diverse academic sources, research papers, and industry best practices",
            "tool_used": [model_name, "ai-research-analysis", "knowledge-synthesis"],
            "timestamp": datetime.now().isoformat(),
            "id": f"research_{datetime.now().timestamp()}",
            "mode": "real_ai",
            "research_quality": "professional",
