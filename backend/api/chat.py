...
def _ensure_seed_users():
    ...
    if not users:
        seed = [
            ...
            {"id": "user_003", "name": "Sarah Johnson", "last_seen": None},
            {"id": "user_004", "name": "Michael Chen", "last_seen": None},
            {"id": "user_005", "name": "Emma Wilson", "last_seen": None},
        ]
...
@router.delete("/peer/message/{message_id}")
async def delete_message(message_id: int, user_id: str):
    """Delete a message - works for both sender and receiver"""
    messages = _read_json(MESSAGES_FILE)
...
async def get_unread_count(user_id: str):
    ...
    total_unread = 0
    for m in messages:
        if m["receiver_id"] == user_id and not m.get("read") and not m.get("deleted"):
            total_unread += 1
            s = m["sender_id"]
